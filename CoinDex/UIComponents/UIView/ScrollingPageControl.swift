...
open class ScrollingPageControl: UIView {
    ...
    private func createViews() {
        dotViews = (0..<pages).map { index in
            delegate?.viewForDot(at: index) ?? CircularView(frame: CGRect(origin: .zero,
                                                                          size: CGSize(width: dotSize,
                                                                                       height: dotSize)))
        }
    }
    ...
    open var dotColor: UIColor = .xFFFFFF.withAlphaComponent(0.5) {
        didSet {
            updateColors()
        }
    }
    ...
    open var selectedColor: UIColor = .xFFFFFF {
        didSet {
            self.updateColors()
        }
    }
    ...
    open var dotSize: CGFloat = 6 {
        didSet {
            ...
            dotViews.forEach { $0.frame = CGRect(origin: .zero,
                                                 size: CGSize(width: dotSize,
                                                              height: dotSize)) }
            invalidateIntrinsicContentSize()
        }
    ...
    internal func updatePositions() {
        ...
        dotViews.enumerated().forEach { page, dot in
            ...
            dot.frame = CGRect(origin: .zero,
                               size: CGSize(width: dotSize * scale,
