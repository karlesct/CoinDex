...
class DeepLinkingTests: XCTestCase {
    ...
    func test_fragment_requiredInt_matches() {
        let recognizer = DeepLinkRecognizer(deepLinkTypes: [FragmentRequiredIntDeepLink.self])
        if let result = recognizer.deepLink(matching: URL(string: "test://#number=42")!) as? FragmentRequiredIntDeepLink {
    ...
    func test_query_with_fragment_requiredInt_matches() {
        let recognizer = DeepLinkRecognizer(deepLinkTypes: [QueryFragmentRequiredIntDeepLink.self])
        if let result = recognizer.deepLink(matching: URL(string: "test://?querynumber=42#fragmentnumber=42")!) as? QueryFragmentRequiredIntDeepLink {
            XCTAssertEqual(result.querynumber, 42)
            XCTAssertEqual(result.fragmentnumber, 42)
        }
        else {
            XCTFail()
        }
...
struct FragmentRequiredIntDeepLink: DeepLink {
    public static let template = DeepLinkTemplate().fragmentStringParameters([.requiredInt(named: "number")])
    public init(values: DeepLinkValues) {
        self.number = values.fragment["number"] as! Int
    }
    let number: Int
}
...
struct QueryFragmentRequiredIntDeepLink: DeepLink {
    public static let template = DeepLinkTemplate()
        .queryStringParameters([.requiredInt(named: "querynumber")])
        .fragmentStringParameters([.requiredInt(named: "fragmentnumber")])
    public init(values: DeepLinkValues) {
        self.querynumber = values.query["querynumber"] as! Int
        self.fragmentnumber = values.fragment["fragmentnumber"] as! Int
    }
    let querynumber: Int
